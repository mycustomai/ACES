...
from experiments.runners.batch_new.providers._base.submit import ChunkingStrategy
from experiments.runners.batch_new.providers.gemini.submit import (
    GeminiBatchProviderSubmitter,
)
from experiments.runners.batch_new.typedefs import (
...
class TestGeminiBatchProviderSubmitter:
    @patch("experiments.runners.batch_new.providers.gemini.submit.genai")
    @patch("experiments.runners.batch_new.providers.gemini.submit.storage")
    @patch("experiments.runners.batch_new.providers.gemini.submit.vertexai")
    def test_setup_success(
        ...
        submitter = GeminiBatchProviderSubmitter(engine_params)
    ...
    def test_setup_missing_config(self, engine_params):
        """Test setup fails with missing configuration."""
        # Clear environment variables
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(ValueError) as exc_info:
                GeminiBatchProviderSubmitter(engine_params)
